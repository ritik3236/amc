...
import { CurrencyInterface, PaymentMethodInterface } from '@/lib/zod';
...
export const convertAmountInMainCurrency = (amount: string | number, currency: CurrencyInterface) => {
    if (!currency) {
        return 0;
    }
...
export const convertAmount = (amount: string | number, fromCurrency: CurrencyInterface, toCurrency: CurrencyInterface) => {
    if (!fromCurrency || !toCurrency) {
        console.error('Currency not found');
...
export const parseJSON = <T>(jsonString: string): T | null => {
    try {
        ...
        console.error('Invalid JSON:', error.message, jsonString);
